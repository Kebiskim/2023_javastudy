...
// 1. 자식클래스가 일반 클래스인 경우 반드시 오버라이딩 해야 한다. (implements)
public class Ex01_extends implements Ex01_interface {

	@Override
	public void sound() {
		System.out.println("음앙듣기");
	}
...
// 2. 자식클래스를 추상클래스로 만들면 오버라이딩을 할 필요가 없다.
// 		객체 생성 시 주의사항 (부모클래스의 추상메서드를 구현하지 않았음)
...
// 3. 자식클래스를 interface로 만들면 오버라이딩을 할 필요가 없다. (extends) 
