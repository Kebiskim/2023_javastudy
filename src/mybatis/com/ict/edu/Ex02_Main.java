			...
			public void actionPerformed(ActionEvent e) {
				...
				if (name.length() > 0 | address.length() > 0 | phone.length() > 0) {
						...
						if (res > 0) {
							...
							JOptionPane.showMessageDialog(getParent(), "삽입 실패! 내용을 입력하세요.");
						}
//					}else {
        ...
        jb3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(custid.length() > 0) {
						Ex02_VO vo = new Ex02_VO();
						vo.setCustid(custid);
						int result = Ex02_DAO.getDelete(vo);
						if (result > 0) {
        ...
        jb4.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(msg.equals("검색")) {
					String custid = jtf1.getText().trim();
					if(custid.length()>0) {
						Ex02_VO vo = Ex02_DAO.getOne(custid);
						prn3(vo);
						jta.setText("");
					...
					int result = Ex02_DAO.getUpdate(vo);
					if (result > 0) {
